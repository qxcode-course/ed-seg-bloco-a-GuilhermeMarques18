package main

import (
	"fmt"

	"github.com/gdamore/tcell/v2"
)

//e.cursor: O nó do caractere atual (onde o foco visual está).
//e.line: O nó da linha atual na lista global de linhas.
//e.line.Value: A lista de caracteres da linha onde o cursor está.
//e.line.Value.End(): O nó sentinela (fim da linha/quebra de linha).
//e.lines.End(): O sentinela da lista de linhas (indica que não há mais nada abaixo)

type Editor struct {
	lines  *List[*List[rune]]
	line   *Node[*List[rune]]
	cursor *Node[rune]
	screen tcell.Screen
	style  tcell.Style
}

func (e *Editor) InsertChar(r rune) {
	e.cursor = e.line.Value.Insert(e.cursor, r)
	e.cursor = e.cursor.Next()
}

func (e *Editor) KeyLeft() {
	if e.cursor != e.line.Value.Front() { // Se o cursor não está no início da linha
		e.cursor = e.cursor.Prev() // Move o cursor para a esquerda
		return
	}
	// Estamos no início da linha
	if e.line != e.lines.Front() { // Se não está na primeira linha
		e.line = e.line.Prev()        // Move para a linha anterior
		e.cursor = e.line.Value.End() // Move o cursor para o final da linha
	}
}

func (e *Editor) KeyEnter() {
	newList := NewList[rune]()

	for e.cursor != e.line.Value.End() {
		newList.PushBack(e.cursor.Value)
		e.cursor = e.line.Value.Erase(e.cursor)
	}

	e.lines.Insert(e.line.Next(), newList)
	e.line = e.line.Next()
	e.cursor = e.line.Value.Front()

}

func (e *Editor) KeyRight() {
	if e.cursor == e.line.Value.End() {
		if e.line.Next() == e.lines.End() {
			return
		}
		e.line = e.line.Next()
		e.cursor = e.line.Value.Front()
		return
	}
	e.cursor = e.cursor.Next()
}

func (e *Editor) KeyUp() {
	if e.line == e.lines.Front() {
		return
	}
	count := 0

	for i := e.line.Value.Front(); i != e.cursor; i = i.Next() {
		count++
	}
	e.line = e.line.Prev()

	e.cursor = e.line.Value.Front()
	for i := 0; i < count; i++ {
		if e.cursor == e.line.Value.End() {
			break
		}
		e.cursor = e.cursor.Next()
	}
}

func (e *Editor) KeyDown() {
	if e.line == e.lines.End() {
		return
	}
	count := 0

	for i := e.line.Value.Front(); i != e.cursor; i = i.Next() {
		count++
	}
	e.line = e.line.Next()
	e.cursor = e.line.Value.Front()

	for i := 0; i < count; i++ {
		if e.cursor == e.line.Value.End() {
			break
		}
		e.cursor = e.cursor.Next()
	}
}

func (e *Editor) KeyBackspace() {
	if e.cursor != e.line.Value.Front() {
		e.line.Value.Erase(e.cursor.Prev())
		return
	}
	if e.line == e.lines.Front() {
		return
	}
	prevLine := e.line.Prev()
	newCursor := prevLine.Value.End()

	for e.line.Value.Front() != e.line.Value.End() {
		caractere := e.line.Value.Front()

		prevLine.Value.PushBack(caractere.Value)

		e.line.Value.Erase(caractere)
	}

	e.lines.Erase(e.line)

	e.line = prevLine
	e.cursor = newCursor
}

func (e *Editor) KeyDelete() {
	if e.cursor == e.line.Value.End() {
		if e.line.Next() == e.lines.End() {
			return
		}
		nextLine := e.line.Next()

		for nextLine.Value.Front() != nextLine.Value.End() {
			charNode := nextLine.Value.Front()
			e.line.Value.PushBack(charNode.Value)
			nextLine.Value.Erase(charNode)
		}
		e.lines.Erase(nextLine)
		return
	}
	newCursor := e.cursor.Next()
	e.line.Value.Erase(e.cursor)
	e.cursor = newCursor
}

func main() {
	// Texto inicial e posição do cursor
	editor := NewEditor()
	editor.Draw()
	editor.MainLoop()
	defer editor.screen.Fini() // Encerra a tela ao sair
}

func (e *Editor) MainLoop() {
	for {
		ev := e.screen.PollEvent()
		switch ev := ev.(type) {
		case *tcell.EventKey:
			switch ev.Key() {
			case tcell.KeyEsc, tcell.KeyCtrlC:
				return
			case tcell.KeyEnter:
				e.KeyEnter()
			case tcell.KeyLeft:
				e.KeyLeft()
			case tcell.KeyRight:
				e.KeyRight()
			case tcell.KeyUp:
				e.KeyUp()
			case tcell.KeyDown:
				e.KeyDown()
			case tcell.KeyBackspace, tcell.KeyBackspace2:
				e.KeyBackspace()
			case tcell.KeyDelete:
				e.KeyDelete()
			default:
				if ev.Rune() != 0 {
					e.InsertChar(ev.Rune())
				}
			}
			e.Draw()
		case *tcell.EventResize:
			e.screen.Sync()
			e.Draw()
		}
	}
}

func NewEditor() *Editor {
	e := &Editor{}
	// Inicializa a tela
	screen, err := tcell.NewScreen()
	if err != nil {
		fmt.Printf("erro ao criar a tela: %v", err)
	}
	if err := screen.Init(); err != nil {
		fmt.Printf("erro ao iniciar a tela: %v", err)
	}
	e.screen = screen
	e.lines = NewList[*List[rune]]()
	e.lines.PushBack(NewList[rune]())
	e.line = e.lines.Front()
	e.cursor = e.line.Value.Back()
	// Define o estilo do texto (branco com fundo preto)
	e.style = tcell.StyleDefault.Foreground(tcell.ColorWhite).Background(tcell.ColorBlack)

	// Limpa a tela e define o estilo base
	e.screen.SetStyle(e.style)
	e.screen.Clear()
	return e
}

func (e *Editor) Draw() {
	e.screen.Clear()
	x := 0
	y := 0
	for line := e.lines.Front(); line != e.lines.End(); line = line.Next() {
		for char := line.Value.Front(); ; char = char.Next() {
			data := char.Value
			if char == line.Value.End() {
				data = '⤶'
			}
			if data == ' ' {
				data = '·'
			}
			if char == e.cursor {
				e.screen.SetContent(x, y, data, nil, e.style.Reverse(true))
			} else {
				e.screen.SetContent(x, y, data, nil, e.style)
			}
			x++
			if char == line.Value.End() {
				break
			}
		}
		y++
		x = 0
	}
	e.screen.Show()
}
